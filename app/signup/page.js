...
export default function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      addToast("Account created successfully! Welcome to NexAI", "success");
      setTimeout(() => {
        router.push("/");
      }, 1500);
    } catch (err) {
    ...
    <div className="min-h-screen bg-bg flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-md w-full space-y-8 bg-card p-10 rounded-2xl border border-border relative z-10 shadow-2xl fade-up">
        <div className="text-center">
          ...
          <h2 className="mt-6 text-2xl font-bold text-text font-heading">Create your account</h2>
        </div>
        ...
        <p className="mt-8 text-center text-sm text-muted">
          ...
          <Link href="/login" className="font-medium text-primary hover:text-primary-lt transition-colors">
            Login
          </Link>
        </p>
      </div>
